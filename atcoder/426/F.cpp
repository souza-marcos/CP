...
namespace beats {
	...
	node updatemin(int a, int b, ll x, int p=1, int l=0, int r=n-1) {
		if (b < l or r < a or seg[p].ma1 <= x) return seg[p]; // Esses caras já são menores que eu então não preciso atualizar
...
void solve(){
    ...
    while(q--){
        int l, r, k; cin >> l >> r >> k;
        l --, r--;
        int prev = beats::query(l, r).second;
        beats::updatesum(l, r, -k);
        beats::updatemax(l, r, 0ll);
        int after = beats::query(l, r).second;
